...
const UpdateDishesPage = (props) => {
  const storeData = useContext(AppContext);
  const mid = props.values.mid;
  const cid = props.values.cid;
  const index = props.values.index
  const foodMenuData = storeData.foodMenu
  let selectedDish = storeData.foodMenu[mid].categories[cid].dishes[index]
  const navigate = useNavigate();


  const [showServingSizefields, setShowServingSizeFields] = useState(selectedDish.servingSizefieldsList.length>0?(true):(false));
  const [servingSizefieldsList, setServingSizefieldsList] = useState([...selectedDish.servingSizefieldsList]);
  const [isGlutenFree, setIsGlutenFree] = useState(selectedDish.isGlutenFree);
  const [isPopular, setIsPopular] = useState(selectedDish.isPopular);
  const [isVegan, setIsVegan] = useState(selectedDish.isVegan);
  const [isVegetarian, setIsVegetarian] = useState(selectedDish.isVegetarian);
  const [isAvailable, setIsAvailable] = useState(selectedDish.isAvailable);
  const [addOn, setAddOn] = useState(false);

  const [dishName, setDishName] = useState(selectedDish.dishName);
  const [dishDesc, setDishDesc] = useState(selectedDish.dishDesc);
  const [cookingTime,setCookingTime]=useState(selectedDish.cookingTime)

  const [addonName, setAddonName] = useState("");
  const [addonDesc, setAddonDesc] = useState("");
  const [addonPrice, setAddonPrice] = useState(0);
  const [addOnList, setAddOnList] = useState(selectedDish.servingSizefieldsList);

  const [size, setSize] = useState("");
  const [price, setPrice] = useState("");

  const [stdDishPrice, setStdDishPrice] = useState(selectedDish.stdDishPrice);
  const [imgData,setImgData] = useState()
  const [imgUrl,setImgUrl] = useState(selectedDish.imgUrl)
  ...
  const handleSubmit = () => {
    ...
    const dishesData = storeData.foodMenu[mid].categories[cid].dishes
    dishesData.splice(index, 1, dishObj);
  ...
  return (
    <div>
      <Container>
        <Row>
          <Col sm={12}>
            <h1 className="mt-6 mb-5 display-4">Dish Update Form</h1>
          </Col>
        </Row>
                    ...
                    <Col md={{ offset: 2, span: 8 }}>
                      ...
                      <div className="mt-5 ">
                        <Row className="d-flex justify-content-around">
                          ...
                          <Col md={4}>
                            <SwitchBtn1
                              isChecked={showServingSizefields}
                      ...
                      {showServingSizefields.length > 0 ? (
                        <Row className="mt-5">
                      ...
                      <Row className={"mt-2"}>
                        ...
                        {imgUrl?(<img src={imgUrl} className="mb-5"/>):(false)}
...
export default UpdateDishesPage;
